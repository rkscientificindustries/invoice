...
@PageTitle("Invoices")
@Route(value = "invoices-new", layout = MainLayout.class)
public class NewInvoiceListView extends VerticalLayout {
  ...
  public NewInvoiceListView(InvoiceService invoiceService, CustomerService customerService) {
  ...
  private HorizontalLayout buildToolbar() {
    var newBtn = new Button("New Invoice", new Icon(VaadinIcon.PLUS), _ ->
        NewInvoiceView.navigateTo(null)
    );
  ...
  private Grid<Invoice> buildGrid() {
    ...
    grid.addItemClickListener(event ->
        NewInvoiceView.navigateTo(event.getItem().getId())
    );
